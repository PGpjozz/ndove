  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = res.data.filter((o: any) => o.type === "internship");
        setItems(filtered);
      } catch (err) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {loading && <Typography>Loading internships...</Typography>}
          {error && (
          ...
          {!loading && !error && items.length === 0 && (
            <Typography>No internships are currently available.</Typography>
          )}
              ...
              {items.map((item) => (
                <Grid item xs={12} key={item.id}>
                  ...
                  >
                    ...
                    <Button
                      component={Link}
                      to={`/enroll?type=internship&title=${encodeURIComponent(
                        item.title
                      )}`}
                      variant="contained"
                      size="small"
